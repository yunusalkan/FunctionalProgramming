...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int minSwapValue = Result.lilysHomework(Arrays.asList(656085744,
                592976686,
                3037922,
                82266352,
                17574000,
                ...
                106712025,
                286764660));
        System.out.println(minSwapValue);
    }
}
...
class Result {
    ...
    private static int minSwapSortingSmallToHigh(List<Integer> arr) {
        ...
        List<Integer> copyOfSorted = Arrays.stream(sortedArr.toArray(new Integer[arr.size()])).collect(Collectors.toList());
        while (copyOfSorted.size() > 0) {
            int i = sortedArr.indexOf(copyOfSorted.get(0));
            int originalIndex = arr.indexOf(sortedArr.get(i));
            if(i != originalIndex) {
                int searchIndex = originalIndex;
                int counter = 1;
                List<Integer> cycle = new ArrayList<>();
                cycle.add(sortedArr.get(i));
                while ( i != arr.indexOf(sortedArr.get(searchIndex))) {
                    cycle.add(sortedArr.get(searchIndex));
                    searchIndex = arr.indexOf(sortedArr.get(searchIndex));
                    ++counter;
                }
                cycle.add(sortedArr.get(searchIndex));
                cycleList.add(cycle);
                minSwapCount = minSwapCount + (counter);
            }
            else {
                copyOfSorted.remove(sortedArr.get(i));
            }
            copyOfSorted.removeAll(cycleList.stream().flatMap(Collection::stream)
                    .collect(Collectors.toList()));
        }
        System.out.println("Min To High swap Value : " + minSwapCount);
        return minSwapCount;
    }
